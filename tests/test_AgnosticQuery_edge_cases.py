...
class TestAgnosticQueryEdgeCases:
    ...
    def test_configuration_with_invalid_full_text_search_value(self):
        ...
        with pytest.raises(ValueError, match="full_text_search"):
    ...
    def test_reconstruct_at_time_as_sets_no_earlier_snapshot(self):
        ...
        quads = {("<http://s>", "<http://p>", '"o"', "<http://g>")}
        result = _reconstruct_at_time_as_sets(
            prov, quads, ("2021-01-01T00:00:00+00:00", "2021-01-01T00:00:00+00:00")
        )
        assert result == []
    ...
    def test_reconstruct_at_time_as_sets_no_interval_start(self):
        ...
        quads = {("<http://s>", "<http://p>", '"o"', "<http://g>")}
        result = _reconstruct_at_time_as_sets(
            prov, quads, (None, "2021-01-01T00:00:00+00:00")
        )
        assert result == []

    def test_generic_triple_pattern_raises_error(self):
        query = "SELECT ?s ?p ?o WHERE { ?s ?p ?o }"
        with pytest.raises(ValueError, match="specify at least one URI"):
    ...
    def test_collect_triples_flat_with_nested_optional(self):
        query = (
            "SELECT ?s ?o ?v WHERE { ?s <http://ex.com/p> <http://ex.com/o> "
            "OPTIONAL { ?s <http://ex.com/q> ?o "
            "OPTIONAL { ?o <http://ex.com/r> ?v } } }"
        )
