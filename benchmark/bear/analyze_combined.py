...
STYLES = {
    ...
    "tal_hourly": {
        "color": "#0072B2",
        "linestyle": (0, (8, 4)),
        "label": "TAL hourly (1,299 ver.)",
    },
    "ost_daily": {
        "color": "#D55E00",
        "linestyle": "-",
        "label": "OSTRICH daily (89 ver.)",
    },
    "ost_hourly": {
        "color": "#D55E00",
        "linestyle": (0, (8, 4)),
        "label": "OSTRICH hourly (1,299 ver.)",
    },
    "r43_daily": {
        "color": "#009E73",
        "linestyle": "-",
        "label": "R43ples daily (89 ver.)",
    },
    "r43_hourly": {
        "color": "#009E73",
        "linestyle": (0, (8, 4)),
        "label": "R43ples hourly (1,299 ver.)",
    },
}
...
def _plot_line_chart(
    ...
    hourly_r43_data: dict[int, float] | None = None,
) -> None:
    pct_d, vals_d = _normalize_keys(daily_data)
    pct_h, vals_h = _normalize_keys(hourly_data)
    _plot_line(ax, pct_d, vals_d, "tal_daily")
    _annotate_line_range(
        ax, pct_d, vals_d, STYLES["tal_daily"]["color"], x_pos=20, below=True
    )
...
def plot_vm_combined(
    daily_data: dict,
    daily_ost: list[Path],
    hourly_data: dict,
    hourly_ost: list[Path],
    plot_dir: Path,
    daily_r43: Path | None = None,
    hourly_r43: Path | None = None,
) -> None:
    fig, ax = plt.subplots(figsize=(8, 5))
    ost_d = (
        load_ostrich_vm_by_version(daily_ost)
        if any(f.exists() for f in daily_ost)
        else None
    )
    ost_h = (
        load_ostrich_vm_by_version(hourly_ost)
        if any(f.exists() for f in hourly_ost)
        else None
    )
...
def plot_dm_combined(
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    ost_d = (
        load_ostrich_dm_by_version(daily_ost)
        if any(f.exists() for f in daily_ost)
        else None
    )
    ost_h = (
        load_ostrich_dm_by_version(hourly_ost)
        if any(f.exists() for f in hourly_ost)
        else None
    )
...
def plot_vq_combined(
    ...
    bars_d = ax.bar(x - width / 2, daily_vals, width, color=colors, edgecolor="black")
    bars_h = ax.bar(
        x + width / 2, hourly_vals, width, color=colors, edgecolor="black", hatch="//"
    )
